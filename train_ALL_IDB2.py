...
parser.add_argument('--num_classes', type=int, default=2, help='the number of classes')  # 分类数量
...
parser.add_argument('--lr', type=float, default=0.0005, help='star learning rate')
# parser.add_argument('--lr', type=float, default=0.0001, help='star learning rate')  # default
...
# parser.add_argument('--data_path', type=str, default=r"./data/ALL_IDB2")
parser.add_argument('--data_path', type=str, default=r"./data/ALL_IDB2")
parser.add_argument('--model', type=str, default="vision_transformer_small", help=' select a model for training')
...
def main(args):
    ...
    if opt.tensorboard:
        ...
        log_path = os.path.join('./results/tensorboard/ALL_IDB2', args.model)
    ...
    # save_path = os.path.join(os.getcwd(), 'results/weights_pretraind/ALL_IDB2', args.model)
    save_path = os.path.join(os.getcwd(), 'results/weights/ALL_IDB2', args.model)
    ...
    for epoch in range(args.epochs):
        ...
        scheduler.step()
        # validate
        val_acc, precision, recall, f1_score, mcc = evaluate(model=model, data_loader=val_loader, device=device, ifIndicators=True, Binary=True)

        print('[epoch %d] train_loss: %.5f  train_acc: %.5f  val_accuracy: %.5f' % (
        epoch + 1, mean_loss, train_acc, val_acc))
        with open(os.path.join(save_path, "kansformer_cell_log.txt"), 'a') as f:
            f.writelines('[epoch %d] train_loss: %.5f  train_acc: %.5f precision:%.5f recall:%.5f f1_score:%.5f val_accuracy: %.5f mcc: %.5f' % (
            epoch + 1, mean_loss, train_acc * 100, precision * 100, recall * 100, f1_score * 100, val_acc * 100, mcc * 100) + '\n')
